...
function preloadIframes() {
    ...
    Object.keys(programData).forEach(key => {
        ...
        if (program.appPath) {
            console.log(`>>> Preloading iframe for: ${key}`);
    ...
    console.log('>>> iframe preloading complete.');
}
...
function getPreloadedIframe(programKey) {
    ...
    if (iframe) {
        ...
        console.log(`>>> Using preloaded iframe for: ${programKey}`);
        return iframe;
    }
    console.warn(`>>> No preloaded iframe found for: ${programKey}. Creating new one.`);
