...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleImageError() {
        ...
        // mainImage.src = "icons/image-error-placeholder.png"; 
        mainImage.src = ""; // Clear src to show alt text more reliably
        // Potentially disable zoom/rotate/pan controls here if needed
    }
    ...
    function showTooltip(element) {
        ...
        const parentRect = document.body.getBoundingClientRect(); // Use body as reference

        tooltipElement.style.left = `${btnRect.left - parentRect.left}px`;
        tooltipElement.style.top = `${btnRect.bottom - parentRect.top + 5}px`; // Position below button

        // Ensure tooltip stays within viewport (simple check)
        ...
        if (tooltipElement.offsetTop + tooltipElement.offsetHeight > window.innerHeight) {
            tooltipElement.style.top = `${btnRect.top - parentRect.top - tooltipElement.offsetHeight - 5}px`; // Position above if no space below
        }
    }
    ...
    document.querySelectorAll('.controls button').forEach(button => {
        ...
        button.addEventListener('click', hideTooltip); // Hide on click too
    });
